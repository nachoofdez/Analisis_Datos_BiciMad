...
def main(sc, years, months, datadir):
         ...
         for m in months:
             ...
             try:
                 if y!=2017 and m!=5:
                     ...
                     rdd = rdd.union(sc.textFile(filename)) #Por si no existe la ruta
             except:
                 pass
     ...
     rdd_base=rdd.map(datos).filter(lambda x: x is not None) #Obtenemos el rdd de partida con el que trabajaremos cada problema
